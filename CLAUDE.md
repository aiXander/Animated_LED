...
The big shift in **surface v2 (the current branch)** is that effects are no longer composed from a typed primitive graph; the LLM authors a Python `Effect` subclass per chat turn and the operator stacks them Resolume-style into a *composition*. The summary below describes the code as it actually exists today.
