...
SSH into Pi via tailscale remotely:
ssh xander@xanderpi
...
Local TADAAM url:
http://192.168.0.200:8000/?password=kaailed
http://192.168.0.200:8766/?password=kaailed

  ┌───────────────────┬──────────────────────────────┐
  │      Service      │ Address from another device  │
  ├───────────────────┼──────────────────────────────┤
  │ ledctl            │ 192.168.0.200:8000           │
  ├───────────────────┼──────────────────────────────┤
  │ audio-server      │ 192.168.0.200:8765 and :8766 │
  ├───────────────────┼──────────────────────────────┤
  │ SSH               │ ssh xander@192.168.0.200     │
  └───────────────────┴──────────────────────────────┘
...
To Fix:
1. The Real Weak Link: The MicroSD Card
This is what trips up almost everyone. Standard MicroSD cards are not designed for the constant, repetitive data writes generated by an operating system (especially system logs like syslog or auth.log). Eventually, a sector wears out, or a sudden power flicker occurs during a write, causing filesystem corruption.

The Fix:

Move the root filesystem to an SSD via USB, or use a high-end NVMe HAT if you are on a Pi 5. SSDs handle heavy write-cycles and random I/O massively better.

If you must use an SD card, buy a high-end "High Endurance" or "Industrial" MLC card (often marketed for dashcams).

Use utilities like log2ram, which write system logs to RAM instead of disk, syncing them only periodically.
...
uv run ledctl run --config config/config.pi.yaml


uv run ledctl run --config config/config.dev.yaml --open
...
uv run ledctl show-config --config config/config.dev.yaml
